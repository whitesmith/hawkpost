...
class CreateBoxForm(ModelForm):
    ...
    field_order = ["name", "never_expires", "expires_at", "max_messages", "description"]

    class Meta:
        model = Box
        fields = ["name", "description", "expires_at", "max_messages", "verified_only"]
    ...
    def clean_expires_at(self):
        ...
        if not expires_at and not never_expires:
            self.add_error(
                "expires_at",
                _("This field is required, unless box is set to " "never expire."),
            )
        return expires_at
...
class SubmitBoxForm(Form):
    ...
    def clean_message(self):
        ...
        if getsizeof(message) > MAX_MESSAGE_SIZE:
            self.add_error(
                "message", _("The message or file exceeds your allowed size limit.")
            )
