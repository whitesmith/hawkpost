...
class UpdateUserInfoForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "keyserver_url": forms.TextInput(
                attrs={"placeholder": _("https://example.com/key.asc")}
            ),
    ...
    current_password = forms.CharField(
        label=_("Current password"), required=False, widget=forms.PasswordInput
    )
    new_password1 = forms.CharField(
        label=_("New password"), required=False, widget=forms.PasswordInput
    )
    new_password2 = forms.CharField(
        label=_("New password confirmation"), required=False, widget=forms.PasswordInput
    )
    ...
    def clean_keyserver_url(self):
        ...
        if url:
            try:
                ...
                self.add_error("keyserver_url", _("Could not access the specified url"))
                return url
            ...
            if 200 <= res.status_code < 300 and begin >= 0 and end > begin:
                ...
                self.add_error("keyserver_url", _("This url does not have a pgp key"))
        return url
